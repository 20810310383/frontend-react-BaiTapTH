...
export const orderSlice = createSlice({
    ...
    reducers: {
        ...
        doDeleteItemCartAction: (state, action) => {
            const productToDelete = state.carts.find(c => c._id === action.payload._id);            
    
            // Lọc sản phẩm khỏi giỏ hàng
            state.carts = state.carts.filter(c => c._id !== action.payload._id);

            if (productToDelete) {
                message.success(`Đã xóa sản phẩm ${productToDelete.detail.mainText} khỏi giỏ hàng`);
            }
        }
